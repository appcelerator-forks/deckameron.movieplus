...
theMovieDb.common = {
    api_key: "0d3d596184fd53f786b23f93129c5321",
    ...
    generateQuery: function(options) {
        ...
        if (Object.keys(myOptions).length > 0) for (option in myOptions) myOptions.hasOwnProperty(option) && "id" !== option && "body" !== option && (query = query + "&" + option + "=" + myOptions[option]);
        return query;
    },
    validateCallbacks: function(callbacks) {
        "use strict";
        if ("function" != typeof callbacks[0] || "function" != typeof callbacks[1]) throw "Success and error parameters must be functions!";
    },
    validateRequired: function(args, argsReq, opt, optReq, allOpt) {
        "use strict";
        var i, allOptional;
        allOptional = allOpt || false;
        if (args.length !== argsReq) throw "The method requires  " + argsReq + " arguments and you are sending " + args.length + "!";
        if (allOptional) return;
        if (argsReq > 2) for (i = 0; optReq.length > i; i += 1) if (!opt.hasOwnProperty(optReq[i])) throw optReq[i] + " is a required parameter and is not present in the options!";
    },
    ...
    client: function(options, success, error) {
        ...
        xhr = Ti.Network.createHTTPClient({
            onload: function() {
                4 === xhr.readyState ? xhr.status === status ? success(xhr.responseText) : error(xhr.responseText) : error(xhr.responseText);
            },
            onerror: function() {
        ...
        "POST" === options.method ? xhr.send(JSON.stringify(options.body)) : xhr.send(null);
    }
};
